...
namespace JeuxDesprit
{
    ...
    public class DatabaseManager
    {
        ...
        /// <summary>
        /// Ajoute un joueur avec son mot de passe dans la base de données
        /// </summary>
        /// <param name="joueur">Joueur à ajouter</param>
        ...
        public int AjouterJoueurAvecMotDePasse(Joueur joueur, string motDePasseHache)
        {
            try
            {
                using (MySqlConnection connection = GetConnection())
                {
                    connection.Open();
                    string type = joueur is Amateur ? "amateur" : (joueur is Professionnel ? "professionnel" : "standard");
                    string query = "INSERT INTO Joueur (nom, email, avatar, type, mot_de_passe) VALUES (@nom, @email, @avatar, @type, @mot_de_passe)";
                    ...
                    using (MySqlCommand command = new MySqlCommand(query, connection))
                    {
                        ...
                        command.Parameters.AddWithValue("@avatar", joueur.GetAvatar());
                        command.Parameters.AddWithValue("@type", type);
                        command.Parameters.AddWithValue("@mot_de_passe", motDePasseHache);
            ...
            {
                Console.WriteLine($"Erreur lors de l'ajout du joueur avec mot de passe : {ex.Message}");
                return -1;
            }
        }
        ...
        /// <param name="motDePasseHache">Mot de passe haché</param>
        /// <returns>Joueur si les informations sont correctes, null sinon</returns>
        public Joueur VerifierConnexion(string email, string motDePasseHache)
        {
            try
            {
                ...
                {
                    connection.Open();
                    string query = "SELECT idJoueur, nom, email, avatar, type FROM Joueur WHERE email = @email AND mot_de_passe = @mot_de_passe";
                    ...
                    using (MySqlCommand command = new MySqlCommand(query, connection))
                    {
                        ...
                        using (MySqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                string emailJoueur = reader.GetString(2);
                                ...
                                if (type.ToLower() == "amateur")
                                {
                                    joueur = new Amateur(nom, emailJoueur, avatar);
                                }
                                else if (type.ToLower() == "professionnel")
                                {
                                    joueur = new Professionnel(nom, emailJoueur, avatar);
                                }
                                else
                                {
                                    joueur = new Joueur(nom, emailJoueur, avatar);
                                }
